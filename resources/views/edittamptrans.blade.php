...
<body>
    <div class="container">
        ...
        <form action="/update/{{$allData->id}}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="nama_pelanggan" class="form-label">Nama Pelanggan</label>
                <input type="text" class="form-control" id="nama_pelanggan" name="nama_pelanggan" value="{{$allData->nama_pelanggan}}">
            </div>
            <div class="mb-3">
                <label for="no_telp_pelanggan" class="form-label">No Telpon</label>
                <input type="text" class="form-control" id="no_telp_pelanggan" name="no_telp_pelanggan" value="{{$allData->no_telp_pelanggan}}" required>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" id="obat_id" name="obat_id" required>
                    ...
                    @foreach($obatData as $obat)
                        <option value="{{ $obat->id }}" obat_id="{{ $obat->obat_id }}" nama_obat="{{ $obat->nama_obat }}" harga_obat="{{ $obat->harga_obat }}" @if($allData->obat_id == $obat->id) selected @endif>{{ $obat->obat_id }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="jumlah_beli" name="jumlah_beli" value="{{$allData->jumlah_beli}}" required oninput="calculateTotal()">
            </div>
            <div class="mb-3">
                <label for="total_harga" class="form-label">Total Harga</label>
                <input type="text" class="form-control" id="total_harga" name="total_harga" value="{{$allData->total_harga}}" required>
            </div>
            <button type="submit" class="btn btn-outline-primary" style="margin-bottom: 3%">Update</button>
            <a href="/tampil" class="btn btn-outline-warning" style="margin-bottom: 3%">Kembali</a>
        </form>
    </div>
        </div>
    </div>
